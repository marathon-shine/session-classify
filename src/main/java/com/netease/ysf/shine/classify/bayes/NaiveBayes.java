...
public class NaiveBayes implements OnlineClassifier<double[]>, SoftClassifier<double[]> {
    ...
    public int[] predictTop(double[] x) {
        if (x.length != p) {
            throw new IllegalArgumentException(String.format("Invalid input vector size: %d", x.length));
        }

        List<Label> topLabels = initTopLabels(3);
        boolean any = model == Model.GENERAL ? true : false;
        for (int i = 0; i < k; i++) {
            double logprob = Math.log(priori[i]);
            ...
            if (any) {
                addLabel(topLabels, logprob, i);
            }
        }
        return topLabels(topLabels);
    }

    private List<Label> initTopLabels(int count) {
        List<Label> topLabels = new ArrayList<>(count);
    ...
    private int[] topLabels(List<Label> labelList) {
        labelList.sort((l, r) -> l.prob > r.label ? 1 : (l.prob == r.prob ? 0 : -1));
